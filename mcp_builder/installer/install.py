...
def install_command(
    ...
    *,
    ...
    force: bool
        If True, overwrite an existing installation at the same alias/version.
        If False and the target exists, treat as success (idempotent).
    """
    ...
    try:
        ...
        # If target exists: idempotent by default, or overwrite on --force
        if final_dir.exists():
            if force:
                shutil.rmtree(final_dir, ignore_errors=True)
            else:
                # Use existing install as success path
                if probe:
